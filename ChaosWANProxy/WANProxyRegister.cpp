...
#define INFO    INFO_LOG(WANProxyRegister)
#define ERROR   ERR_LOG(WANProxyRegister)
#define DBG     DBG_LOG(WANProxyRegister)
...
WANProxyRegister::WANProxyRegister():
rpc_domain("agent"),
registration_state(WANProxyRegisterStateUnregistered),
mds_message_channel(NULL),
reg_retry_counter(0),
max_reg_retry_counter(5){
    //!register RPC action
    DeclareAction::addActionDescritionInstance<WANProxyRegister>(this,
                                                              &WANProxyRegister::registrationACK,
...
WANProxyRegister::~WANProxyRegister() {}
void WANProxyRegister::addWorker(WorkerSharedPtr new_worker) {
...
void WANProxyRegister::init(void *init_data)  {
    //add all agent
...
void WANProxyRegister::start()  {
    //register rpc action
    NetworkBroker::getInstance()->registerAction(this);
    //start the registering state machine
    registration_state() = WANProxyRegisterStateStartRegistering;
    AsyncCentralManager::getInstance()->addTimer(this,
                                                 SM_EXECTION_STEP_MS,
                                                 SM_EXECTION_STEP_MS);
}

void WANProxyRegister::stop()  {
    //register rpc action
    NetworkBroker::getInstance()->deregisterAction(this);
    
    registration_state() = WANProxyRegisterStateStartUnregistering;
    try{
    AsyncCentralManager::getInstance()->addTimer(this,
                                                 SM_EXECTION_STEP_MS,
                                                 SM_EXECTION_STEP_MS);
    while(registration_state != WANProxyRegisterStateUnregistered &&
          registration_state != WANProxyRegisterStateFault) {usleep(500000);}
    }catch(...) {
...
CDWUniquePtr WANProxyRegister::registrationACK(CDWUniquePtr  ack_pack) {
    ...
    if(ack_pack->hasKey(AgentNodeDomainAndActionRPC::REGISTRATION_RESULT)) {
        WANProxyRegisterStateLockableWriteLock write_lock = registration_state.getWriteLockObject();
        ...
        switch(ack_val){
            case ErrorCode::EC_MDS_NODE_REGISTRATION_OK:
                registration_state() = WANProxyRegisterStateRegistered;
...
ChaosUniquePtr<chaos::common::data::CDataWrapper> WANProxyRegister::getAgentRegistrationPack() {
...
void WANProxyRegister::timeout() {
    const std::string& agent_uid = ChaosAgent::getInstance()->settings.agent_uid;
    WANProxyRegisterStateLockableWriteLock write_lock = registration_state.getWriteLockObject();
    switch(registration_state()) {
        case WANProxyRegisterStateUnregistered:
            ...
            break;
        case WANProxyRegisterStateStartRegistering: {
            reg_retry_counter = 0;
            //add healt metric for newly create control unit instance
            HealtManager::getInstance()->addNewNode(agent_uid);
            registration_state() = WANProxyRegisterStateRegistering;
        }
        case WANProxyRegisterStateRegistering: {
        ...
        case WANProxyRegisterStateRegistered: {
            HealtManager::getInstance()->addNodeMetricValue(agent_uid,
                                                            NodeHealtDefinitionKey::NODE_HEALT_STATUS,
            ...
            try{
                ...
                registration_state() = WANProxyRegisterStateFault;
            }
            break;
        }
        case WANProxyRegisterStateStartUnregistering: {
            HealtManager::getInstance()->addNodeMetricValue(agent_uid,
                                                            NodeHealtDefinitionKey::NODE_HEALT_STATUS,
                                                            NodeHealtDefinitionValue::NODE_HEALT_STATUS_UNLOADING);
            registration_state() = WANProxyRegisterStateUnregistering;
        }
        case WANProxyRegisterStateUnregistering:
            reg_retry_counter = 0;
            registration_state() = WANProxyRegisterStateUnregistered;
            for(MapWorkerIterator iter = map_worker.begin();
                iter != map_worker.end();
        ...
        case WANProxyRegisterStateFault:
            HealtManager::getInstance()->addNodeMetricValue(agent_uid,
                                                            NodeHealtDefinitionKey::NODE_HEALT_STATUS,
