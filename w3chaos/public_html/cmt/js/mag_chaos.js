...
$(document).ready(function() {
    ...
    function element_sel(field, arr, add_all) {
        ...
        $(arr).each(function(i) {
            if (arr[i] != "ACCUMULATOR") { //tolgo dalla lista la zona accumulatore
                if (arr[i] != "SCRAPER") { //tolgo dalla lista gli scraper
    ...
    function add_element(arr) {
        ...
        $(arr).each(function(i) {
            ...
            $("#main_table").append("<tr class='col s12 m12 l12 tr_element' id='tr_element_" + i + "'><td class='col s2 m2 l2 td_element td_name' id='name_element_"
                                    + [i] + "'>" + arr[i] + "</td><td class='col s2 m2 l2 td_element' id='td_readout_" + [i]
                                    + "'> 0</td><td class='col s1 m1 l1 td_element' id='td_settCurr_"
                                    + [i] + "'>0</td><td style='border-right:hidden;' class='col s1 m1 l1 td_element' id='td_saved_curr_" + [i]
                                    + "'></td><td style='border-right:hidden;border-left:hidden;' class='col s1 m1 l1 td_element' id='td_saved_state_" + [i]
                                    + "'></td><td style='border-left:hidden;' class='col s1 m1 l1 td_element' id='td_saved_pola_" + [i]
                                    + "'></td><td style='border-right:hidden;' class='col s1 m1 l1 td_element' id='td_flag_state_" + [i]
                                    + "'></td><td style='border-right:hidden;' class='col s1 m1 l1 td_element' id='td_error_state_" + [i]
                                    + "'></td><td style='border-right:hidden;border-left:hidden;' class='col s1 m1 l1 td_element' id='td_flag_pol_" + [i]
                                    + "'></td><td style='border-left:hidden;' class='col s1 m1 l1 td_element' id='td_flag_rl_" + [i] +"'></td></tr>");
            });
            ...
            $.get("http://" + location.host + ":8081/CU?dev="+ url_mag + "&cmd=channel&parm=-1", function(datavalue, textStatus) {
                ...
                try {
                   ...
                   /* for(var i = 0; i < colm_fl_state.length; i++) {
                    ...
                    for(var i = 0; i < colm_rem_loc.length; i++) {
                        if (colm_rem_loc[i] == "true") {
                            $("#td_flag_rl_" + i).html('<i class="material-icons red" id="local_' + [i] + '">vpn_key</i>');
                 ...
                 /*   for(var i = 0; i<colm_alarm.length; i++) {
                    ...
                    for(var i = 0; i < err_state.length; i ++) {    ///*Attiva l'alert se c'è un errore di sistema ***/
                        if (err_state[i] != "") {
